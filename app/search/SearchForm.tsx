...
function normalizeHandleInput(raw: string) {
  // "8841 john" -> "8841-john" (but we keep it as user-facing in the input)
  return raw
}
...
export default function SearchForm({
  ...
  const [value, setValue] = useState(normalizeHandleInput(initialQuery))
  ...
  const [cityLimit, setCityLimit] = useState<number>(80)
  ...
  // Fetch city suggestions when state is chosen and city dropdown is open
  useEffect(() => {
    if (!statePicked) {
      setCitySuggestions([])
      setCityOpen(false)
  ...
  // Submit as GET query params (so SearchPage can read + pass to CreateDriverForm if missing)
  // NOTE: the actual lookup still uses q only for now (precise handle)
  return (
    <form action="/search" method="get" className="mt-6 w-full max-w-xl space-y-3">
      {/* Main handle input */}
      <input
        name="q"
        value={value}
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <div ref={cityBoxRef} className="relative">
          <input
            ...
            className={[inputClass, !statePicked ? 'opacity-60' : ''].join(' ')}
          />
